...
$newMembre->type = $membre->type;

$_SESSION['MessageGestionCompte']='';
//$newMembre->affiche();
// verification unicité
if (($membre->login != $newMembre->login) || ($membre->email != $newMembre->email)) {
    // login ou password modifié
    // on verifie l'unicite
...
// dans les autres cas, on modifie le user
$_SESSION['typeContenu'] = 'recette';
$membre->update($membre->id_membre, $newMembre->login, $newMembre->password, $newMembre->nom, $newMembre->prenom, $newMembre->email, $newMembre->type);
